...
| A [Nerd Font](https://www.nerdfonts.com/) in the terminal | the git-branch icon in the Claude Code status line, and Powerlevel10k's glyphs | no — the icon renders as a missing-glyph box without it | https://github.com/romkatv/powerlevel10k#fonts |
...
| `claude/statusline-command.sh` | `~/.claude/statusline-command.sh` | Claude Code status line: `user@host` (only as root or over SSH, like the p10k context segment), path, git branch and state (`⇡⇣ ~ + ! ?`), a `wt:<name>` tag inside a linked git worktree, context-usage bar, model with effort level. A second row lists directories added with `/add-dir` (absent when there are none). Styled after the Powerlevel10k prompt in `zsh/.p10k.zsh`; on narrow terminals drops effort, then the worktree tag, then the model, then `user@host` |
